...
case $ab in
    [Aa]*)  USE_BIG_SWARM=false ;;  # Loại bỏ `break` ở đây
    [Bb]*)  USE_BIG_SWARM=true ;;   # Loại bỏ `break` ở đây
    *)      echo ">>> Please answer A or B." ;;
esac

# Gán giá trị cho SWARM_CONTRACT tùy vào lựa chọn
...
case $pc in
    0.5 | 1.5 | 7 | 32 | 72) PARAM_B=$pc ;;  # Loại bỏ `break` ở đây
...
try_ngrok() {
    ...
    if install_ngrok; then
        ...
        echo -e "\n${CYAN}${BOLD}[✓] Starting ngrok...${NC}"
        ngrok http "$PORT" --log=stdout --log-format=json > ngrok_output.log 2>&1 &
        TUNNEL_PID=$!
        sleep 5

        # Kiểm tra log ngrok để xác định lỗi
        tail -n 20 ngrok_output.log
